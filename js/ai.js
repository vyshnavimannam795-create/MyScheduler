  ...
  async _getPredefinedReply(text) {
    ...
    if (this.type === 'owner') {
      ...
      if (query.includes('most requests') || query.includes('slots have most')) {
        try {
          const { data } = await db.from('meetings').select('start_time, end_time').eq('status', 'pending');
          if (!data || data.length === 0) {
            return "There are no pending requests to evaluate slot demand.";
          }
          const counts = {};
          data.forEach(m => {
            const key = formatTimeRange(m.start_time, m.end_time);
            counts[key] = (counts[key] || 0) + 1;
          });
          const sorted = Object.entries(counts).sort((a,b) => b[1] - a[1]);
          const list = sorted.map(([time, count]) => `• ${time}: ${count} request(s)`).join('\n');
